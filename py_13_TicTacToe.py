...
selected_symbol = input(
    'First Player which symbol do you want to play with?(X or 0) ').upper()
...
# make sure the step graphicly align in the 3x3 matrix
playchar = ' ' + selected_symbol + ' '
...
def win():
    ...
    if first_row[0] in [playchar, second_playchar] and first_row[0] == first_row[1] == first_row[2] or \
        ...
        current_table()
        print()
        print('Congratulations ' + player[i] + '! What a bloody legend!')
        print()
        sys.exit()
    # COLUMNS
...
while len(user_move_list) < 9:
    ...
    if len(move) == 2 and move not in user_move_list and move[0] in ['A', 'B', 'C'] and move[1] in ['1', '2', '3']:
        # for validation, make sure the user not moving to an already allocated position
        user_move_list = user_move_list + [move]
        if i == 0:
            first_user_moves()
            if win():
                print()
